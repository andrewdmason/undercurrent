...
import { Send, MessageSquare, AlertTriangle, Check, Settings2, Trash2, Sparkles } from "lucide-react";
...
export type ChatMode = "onboarding" | "refinement";

interface ChatPanelProps {
  ideaId: string;
  projectSlug: string;
  mode: ChatMode;
  /** Called when the AI signals readiness to generate assets (onboarding mode only) */
  onReadyToGenerate?: (summary: string) => void;
  /** Called when script is updated via tool */
  onScriptUpdate?: (script: string) => void;
  /** Called when idea is regenerated */
  onIdeaRegenerate?: () => void;
  /** Called when a tool call starts (for loading states) */
  onToolCallStart?: (toolName: string) => void;
  /** Called when a tool call ends */
  onToolCallEnd?: (toolName: string) => void;
  /** Variant affects layout - main is larger, sidebar is compact */
...
export function ChatPanel({
  ...
  // Determine if this is the main/large variant
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const toolResults: Array<{ name: string; result: { success: boolean; message?: string; error?: string; summary?: string } }> = [];
            ...
            if (data.type === "text") {
              ...
              if (data.result.success) {
                if (data.name === "update_script") {
                  ...
                  onIdeaRegenerate?.();
                  router.refresh();
                } else if (data.name === "ready_to_generate") {
                  // Onboarding mode: AI is ready to generate assets
                  onReadyToGenerate?.(data.result.summary || "");
                }
              }
            } else if (data.type === "error") {
              throw new Error(data.error);
  ...
  }, [inputValue, currentChat, isSending, ideaId, mode, router, onToolCallStart, onToolCallEnd, onScriptUpdate, onIdeaRegenerate, onReadyToGenerate]);
  ...
  if (isLoading) {
    return (
      <div className={cn(
        "flex-1 flex flex-col min-h-0 rounded-lg border border-[var(--border)] bg-[var(--grey-0)] overflow-hidden",
        className
      )}>
        <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--border)]">
          ...
          <h4 className="text-xs font-semibold text-[var(--grey-600)] uppercase tracking-wider">
            {mode === "onboarding" ? "Let's Plan Your Video" : "AI Chat"}
          </h4>
        </div>
  ...
  return (
    <div className={cn(
      ...
      {/* Messages */}
      <div className={cn(
        "flex-1 overflow-auto space-y-3",
        isMain ? "p-6" : "p-3"
      )}>
        {messages.length === 0 && !streamingMessage ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-4">
            <div className={cn(
              "inline-flex items-center justify-center rounded-xl bg-[var(--grey-50)] mb-3",
              isMain ? "w-14 h-14" : "w-10 h-10"
            )}>
              {mode === "onboarding" ? (
                <Sparkles className={cn("text-[var(--cyan-600)]", isMain ? "h-7 w-7" : "h-5 w-5")} />
              ) : (
                <MessageSquare className={cn("text-[var(--grey-300)]", isMain ? "h-7 w-7" : "h-5 w-5")} />
              )}
            </div>
            <p className={cn(
              "text-[var(--grey-500)]",
              isMain ? "text-sm max-w-[280px]" : "text-xs max-w-[180px]"
            )}>
              {mode === "onboarding" 
                ? "I'll ask a few questions to understand your unique perspective on this video."
      ...
      <div className={cn(
        ...
        <div className={cn(
          "flex items-end gap-2 rounded-lg bg-[var(--grey-50)] border border-[var(--border)] focus-within:ring-2 focus-within:ring-[var(--cyan-600)] focus-within:border-transparent",
          isMain ? "px-4 py-3" : "px-3 py-2"
        )}>
          <textarea
            id="chat-input"
            ref={inputRef}
            ...
            placeholder={mode === "onboarding" ? "Share your thoughts..." : "Ask AI to refine the script..."}
            rows={1}
            className={cn(
              "flex-1 bg-transparent text-[var(--grey-800)] placeholder:text-[var(--grey-400)] outline-none resize-none min-h-[18px] max-h-[100px]",
          ...
          >
            <Send className={isMain ? "h-4 w-4" : "h-3.5 w-3.5"} />
          </Button>
        </div>
      </div>
    </div>
